...
class Environment:
    def __init__(self, width: int, height: int):
        ...
        self.obstacles: Set[Tuple[int, int]] = set()  # Start empty
...
def run_simulation_with_graphics():
    ...
    pygame.display.set_caption("Smart Reflex Agent with Mouse Control")
    ...
    running = True
    steps = 0
    paused = True  # Start paused for manual setup
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Left click = move goal
                if event.button == 1:
                    if (grid_x, grid_y) not in env.obstacles and clicked_pos != env.agent_pos:
                        env.goal_pos = clicked_pos
                        print(f"📍 Goal moved to {clicked_pos}")

                # Right click = toggle obstacle
                elif event.button == 3:
                    if clicked_pos != env.agent_pos and clicked_pos != env.goal_pos:
                        if (grid_x, grid_y) in env.obstacles:
                            env.obstacles.remove((grid_x, grid_y))
                            print(f"❌ Obstacle removed at {clicked_pos}")
                        else:
                            env.obstacles.add((grid_x, grid_y))
                            print(f"🧱 Obstacle added at {clicked_pos}")

            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    paused = not paused  # Toggle simulation

        if not paused:
            state = env.get_state()
            if state['at_goal']:
                print(f"\n🎯 Goal reached in {steps} steps!")
                paused = True  # Pause when goal is reached
                continue
            ...
            steps += 1
            clock.tick(5)  # Slow speed for visual clarity
